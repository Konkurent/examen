...
$(document).ready(function(){
  ...
  $('.header__btn, .call__btn_mob').click(function(){
    ...
    $('.call__btn').on('click', function (ev) {
      ev.preventDefault();
      jQuery.ajax({
        url: '/php/ajax/send.php',
        type: "POST",
        dataType: "html",
        data: $(".call_container .form").serialize(),
        success: function(response) {
          result = JSON.parse(response);
          if (result == 'Нет номера телефона:(') {
            alert(result);
          } else {
            alert(result);
  ...
  $('.ex__btn').click(function(){
    ...
    $('.project__btn').on('click', function (ev) {
      ev.preventDefault();
      if (isEmail($(".project__container input[name='Email']").val())) {
        jQuery.ajax({
          url: '/php/ajax/send.php',
          type: "POST",
          dataType: "html",
          data: $(".project__container .form").serialize(),
          success: function(response) {
            result = JSON.parse(response);
            if (result) {
              alert(result);
